...
\textbf{Instructor}: $if(instructor)$$instructor$$else$TBD$endif$ & \textbf{E-mail}: $if(email)$\texttt{$email$}$else$TBD$endif$\\
\textbf{Office Hours}: $if(officehours)$$officehours$$else$TBD$endif$  &  \textbf{Time}: $if(time)$$time$$else$TBD$endif$\\
\textbf{Web}: $if(web)$\href{http://$web$}{\tt $web$}$else$TBD$endif$  & \textbf{Place}: $if(place)$$place$$else$TBD$endif$\\
	&  \\
	\hline
	\end{tabular*}
